...
void Menu(int argc, char *argv[])
{
   ...
   beta = 4;
   mu = beta/kappa_sigma_r;
   c = 0.5*pow((pow(mu,2)+2*beta*mu),0.5);
   delta = 2*c + mu + beta;
   b = 0.5*pow(((pow(delta,2)-pow(beta,2))/delta),0.5);
   k = log(delta/beta);
   ev0 = pow((4*PI/delta),0.5);
   C0 = 0.5*pow(PI/c,-0.25);
